...
#ifdef INVARIANTS_CHECK
        static void CheckInvariants(std::shared_ptr<Node> node, std::vector<T>& values,
...
RedBlackTree<T>::GetNearestLeaf(std::shared_ptr<Node> node) {
...
bool RedBlackTree<T>::Erase(const T& value) {
    ...
    while (true) {
        ...
        std::shared_ptr<Node> sibling =
                (kid == Kid::LEFT ? node->parent->right : node->parent->left);
        if (sibling->color == Color::RED) {
...
template<typename T>
void RedBlackTree<T>::Node::CheckInvariants(std::shared_ptr<Node> node, std::vector<T>& values,
                                            std::vector<int32_t>& depths, int32_t black_depth) {
    if (!node) {
